...
UAkComponent::UAkComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {
    ...
    this->bDisableRoomIdentification = false;
    this->bDisableTritonAcoustics = false;
    this->bDisableTritonAcousticsOcclusion = false;
...
float UAkComponent::GetMaxAttenuationRadius() const {
    return 0.0f;
}

float UAkComponent::GetCurrentOrCachedAttenuationRadius() const {
    return 0.0f;
}
